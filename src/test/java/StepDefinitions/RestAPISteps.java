...
public class RestAPISteps implements En {
	...
	public RestAPISteps(DIContext context) {
		...
		/*
		 * Given("I store fake creds in context", () -> { try { // Fetch creds creds =
		 * scenarioContext.GetCreds(); Iterator<String> keys =
		 * creds.keySet().iterator(); while (keys.hasNext()) { String key = keys.next();
		 * System.out.println("Got Creds :" + key + " with password: " +
		 * creds.get(key)); } this.scenarioContext.GetExtentTest().
		 * pass("Inside 'I store fake creds in context'"); } catch (Exception e) {
		 * this.scenarioContext.GetExtentTest().
		 * fail("Inside 'I store fake creds in context'");
		 * this.scenarioContext.IncrementStepErrorCount(e); } });		 
		 */
	}

	@Given("I store fake creds in context")
	public void IStoreFakeCredsInCtx() {
		try {
			// Fetch creds
	...
	public void ITransformArrayOfPojoToJsonFile() {
		try {
	...
	public void IReadTheFileAndModifyValues(String fileName, String name, String job) {
		try {
	...
	@Given("I invoke REST call {string} on {string}")
	public void IInvokeRestCall(String method, String url) {
	...
	@When("I read and parse the Json file {string}")
	public void IReadAndParseJsonFile(String fileName) {
	...
	public void IReadJsonFileAndModifyValues(String fileName, io.cucumber.datatable.DataTable dataTable)
			throws Exception {
		...
		this.scenarioContext.GetExtentTest().pass("inside 'I read the Json file {string}, modify it with values'");
	}

	@When("I invoke REST call {string} on the url")
	public void IInvokeRestCallOntheUrl(String method) {
		Response response = null;
		switch (method.toLowerCase()) {
		case "get":
	...
	public void IPrintResponseContent() {
	...
	@Then("I filter all names starting with {string} from the response to print them")
	public void IFilterallNamesStartingWithFromResponse(String inputName) {
	...
	@Then("I store the url for name={string} in scenario context")
	public void IStoreTheUrlForName(String name) {
		String contents = scenarioContext.GetResponse().getBody().asString();
		JSONObject jobj = new JSONObject(contents);
		JSONArray results = jobj.getJSONArray("results");

		List<JSONObject> filteredName = StreamSupport.stream(results.spliterator(), false).map(r -> (JSONObject) r)
				.filter(u -> u.getString("name").toLowerCase().equals(name.toLowerCase())).collect(Collectors.toList());
	...
	@Then("I verify ResponseCode={string}")
	public void IVerifyResponseCode(String expCode) {
	...
	public void IVerify(String path, String value) {
	...
	@Then("I verify in response {string}={string}")
	public void IVerifyInResponse(String path, String value) {
	...
	public void IFilterToStoreInScenarioContext(String path, Integer filterVal, String pathToStore) {
		String actual_val = new JsonHelper(scenarioContext).ParseJsonFromResponseToFilterValue(path, filterVal,
				pathToStore);
