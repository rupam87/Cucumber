@UI @UI1
Feature: UI feature 1 for test

  Scenario: F1S1 :: Dummy UI scenario - Practice
    Given I navigate to "https://www.rahulshettyacademy.com/AutomationPractice/"
     When I click on "Login" button
     Then I should be at "https://www.rahulshettyacademy.com/AutomationPractice/"

  Scenario: F1S2 :: Dummy UI scenario - Home
    Given I navigate to "https://www.rahulshettyacademy.com/AutomationPractice/"
     When I click on "Home" button
     Then I should be on "https://www.rahulshettyacademy.com/#/index"

  Scenario: F1S3 :: Dummy UI scenario - Practice
    Given I navigate to "https://www.rahulshettyacademy.com/AutomationPractice/"
     When I click on "Login" button
     Then I should be at "https://www.rahulshettyacademy.com/AutomationPractice/"

  Scenario: F1S4 :: Dummy UI scenario - Home
    Given I navigate to "https://www.rahulshettyacademy.com/AutomationPractice/"
     When I click on "Home" button
     Then I should be on "https://www.rahulshettyacademy.com/#/index"

  Scenario: F1S5 :: Dummy UI scenario - Practice
    Given I navigate to "https://www.rahulshettyacademy.com/AutomationPractice/"
     When I click on "Login" button
     Then I should be on "https://www.rahulshettyacademy.com/AutomationPractice/"

  Scenario: F1S6 :: Dummy UI scenario - Home
    Given I navigate to "https://www.rahulshettyacademy.com/AutomationPractice/"
     When I click on "Home" button
     Then I should be at "https://www.rahulshettyacademy.com/#/index"
